...
class align(object):
    ...
    def fill_in_matrix(self):
        ...
        for column in np.arange(len(self.seq_A) + 1):
            self.scoring_matrix.ix[0, column] = 0
        for row in np.arange(len(self.seq_B) + 1):
            self.scoring_matrix.ix[row, 0] = 0

        # Iterate over rows and fill in the matrix based on substitution matrix
        for row_index, row in self.scoring_matrix.iterrows():
            for column_index in np.arange(len(row)):
                if row_index > 0 and column_index > 0:
                    ...
                    score_up = self.scoring_matrix.ix[row_index - 1, column_index] + gap_penalty
                    score_left = self.scoring_matrix.ix[row_index, column_index] + gap_penalty
                    score_diag = self.scoring_matrix.ix[row_index - 1, column_index - 1] + similarity_score
                    ...
                    self.scoring_matrix.ix[row_index, column_index] = final_score if final_score > 0 else 0

        self.scoring_matrix.to_csv('SCORE_current.csv')

    def traceback(self):
        # Find maximum score in matrix

        row_max_indicies = self.scoring_matrix.max(axis=0)
        seq_B_high_score = row_max_indicies.idxmax(axis=0)
        seq_A_high_score = self.scoring_matrix.idxmax(axis=0)
